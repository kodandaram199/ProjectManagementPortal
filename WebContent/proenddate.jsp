...
<div class="jumbotron">
			...
			PreparedStatement st1 = con1.prepareStatement("SELECT enddate FROM tbl_projectdetails WHERE  enddate=?");
			st1.setDate(1,a);
			ResultSet rs1=st1.executeQuery();
			if(rs1.next())
			{
			out.println("<h1 style='text-align:center'>Project Details Of project with enddate "+rs1.getString("enddate"));
			}
			else
			{
				
			    out.println("<h1 style='text-align:center'>No Projects with enddate "+a);
        ...
        <%@ page import="java.sql.*" %> 
			...
			PreparedStatement st = con.prepareStatement("SELECT p.projectname,p.startdate,p.enddate,p.actualcomplete,u.fullname,u.username,pr.priority,w.status AS projectworkstatus,c.clientname,p.projectdesc,p.amount,s.status FROM tbl_projectdetails p,tbl_status s,tbl_workstatus w,tbl_clients c,tbl_priority pr,tbl_users u WHERE p.status=s.statusid  AND p.projectworkstatus=w.statusid AND p.clientid=c.clientid AND p.priority=pr.priorityid AND p.mgrid=u.userid and p.enddate=?");	
			st.setDate(1,a);
