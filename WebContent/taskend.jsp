...
<div class="jumbotron">
			...
			PreparedStatement st1 = con1.prepareStatement("SELECT enddate FROM tbl_tasks WHERE  enddate=?");
			st1.setDate(1,a);
			ResultSet rs1=st1.executeQuery();
			if(rs1.next())
			{
			out.println("<h1 style='text-align:center'>Task Details Of project with enddate "+rs1.getString("enddate"));
			}
			else
			{
				
			    out.println("<h1 style='text-align:center'>No Tasks with enddate "+a);
        ...
        <%@ page import="java.sql.*" %> 
			...
			PreparedStatement st = con.prepareStatement("SELECT t.taskname,p.projectname,pr.priority,w.status AS taskstatus,t.enddate,t.startdate,t.actualcomplete,t.description,u.username,u.fullname,s.status FROM tbl_tasks t,tbl_projectdetails p,tbl_status s,tbl_workstatus w,tbl_priority pr,tbl_users u WHERE t.projectid=p.projectid AND t.status=s.statusid  AND t.taskstatus=w.statusid  AND t.priority=pr.priorityid AND t.employeeid=u.userid AND t.enddate=?");	
			st.setDate(1,a);
