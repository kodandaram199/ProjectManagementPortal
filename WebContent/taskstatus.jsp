...
<div class="jumbotron">
			...
			PreparedStatement st1 = con1.prepareStatement("SELECT w.status FROM tbl_tasks t,tbl_workstatus w WHERE  t.taskstatus=w.statusid AND t.taskstatus="+a);			
			ResultSet rs1=st1.executeQuery();
			if(rs1.next())
			{
			out.println("<h1 style='text-align:center'>Details Of task with status "+rs1.getString("status"));
			}
			else
			{
			    PreparedStatement st2 = con1.prepareStatement("SELECT status from tbl_workstatus where statusid="+a);			
				ResultSet rs2=st2.executeQuery();
				while(rs2.next())
				{
			    out.println("<h1 style='text-align:center'>No tasks with status "+rs2.getString("status"));
				}
			}
