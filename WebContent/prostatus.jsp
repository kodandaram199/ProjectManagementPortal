...
<div class="jumbotron">
			...
			PreparedStatement st1 = con1.prepareStatement("SELECT s.status from tbl_projectdetails p,tbl_status s where p.status=s.statusid and p.status="+a);			
			ResultSet rs1=st1.executeQuery();
			if(rs1.next())
			{
			out.println("<h1 style='text-align:center'>Project Details with status "+rs1.getString("status"));
			}
			else
			{
			    PreparedStatement st2 = con1.prepareStatement("SELECT status from tbl_status where statusid="+a);			
				ResultSet rs2=st2.executeQuery();
				while(rs2.next())
				{
			    out.println("<h1 style='text-align:center'>No Projectswith status "+rs2.getString("status"));
				}
			}
			%>
        	</div>
        ...
        <%@ page import="java.sql.*" %> 
			...
			PreparedStatement st = con.prepareStatement("SELECT p.projectname,p.startdate,p.enddate,p.actualcomplete,u.fullname,u.username,pr.priority,w.status AS projectworkstatus,c.clientname,p.projectdesc,p.amount,s.status FROM tbl_projectdetails p,tbl_status s,tbl_workstatus w,tbl_clients c,tbl_priority pr,tbl_users u WHERE p.status=s.statusid  AND p.projectworkstatus=w.statusid AND p.clientid=c.clientid AND p.priority=pr.priorityid AND p.mgrid=u.userid and p.status="+a);			
